...
define( 'NOTIFIER_THEME_NAME', 'paragon' ); // The theme name
define( 'NOTIFIER_THEME_FOLDER_NAME', 'paragon' ); // The theme folder name
define( 'NOTIFIER_XML_FILE', 'http://www.siiimple.com/updates/notifier.xml' ); // The remote notifier XML file containing the latest version of the theme and changelog
define( 'NOTIFIER_CACHE_INTERVAL', 41600 ); // The time interval for the remote XML cache in the database (21600 seconds = 6 hours)
...
function update_notifier_menu() {  
	if (function_exists('simplexml_load_string')) { // Stop if simplexml_load_string funtion isn't available
		...
		if( (float)$xml->latest > (float)$theme_data['Version']) { // Compare current theme version with the remote XML version
			add_theme_page( NOTIFIER_THEME_NAME . ' Theme Updates', NOTIFIER_THEME_NAME . ' <span class="update-plugins count-1"><span class="update-count2">New Updates</span></span>', 'administrator', 'theme-update-notifier', 'update_notifier');
		}
	}	
}
